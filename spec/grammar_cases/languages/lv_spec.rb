...
describe GrammarCases::Languages::Lv do
  ...
  test_cases.each_pair do|word, options|
    it "has correct grammar cases for #{word} (#{options["description"]})" do
      options["cases"].each_pair do|grammar_case, expectation|
        expect(GrammarCases::Languages::Lv.new(word).to_grammar_case(grammar_case.to_sym, gender: options["gender"])).to eq(expectation)
      end
    end
  end
end
